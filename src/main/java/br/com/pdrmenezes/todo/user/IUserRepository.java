...
public interface IUserRepository extends JpaRepository<UserModel, UUID> {
  // to validate and correctly handle the 'already existing username' we need a
  // method that's not available on JpaRepository, so we'll implement it
  // spring already has autocomplete for functions that we can pass paramaters to
  // such as findByProperty_name
  // as the username is unique, we can change the autocompleted function return
  // type from 'List<UserModel>' to UserModel
